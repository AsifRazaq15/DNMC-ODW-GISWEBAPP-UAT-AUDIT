<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="AssociatedWorkOrders.aspx.cs" Inherits="WebAppForm.AssociatedWorkOrders" %>

<!DOCTYPE html>
<html xmlns="http://www.w3.org/1999/xhtml">
<head runat="server">
    <title>Associated WorkOrders - Vehicles </title>
    <style type="text/css">
        .hiddencol {
            display: none;
        }
...
<body style="background-color: #022130; font-family: Segoe UI Light; color: white;">
    ...
    <form id="form1" runat="server" style="text-align: center;">
        ...
        <div style="text-align: left; float: left; font-family: Segoe UI Light; font-size: Small;">Vehicle: <%=Request.QueryString["Title"]%> -->Associated WorkOrders</div>
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ODWConnectionString %>"
            SelectCommand="SELECT wv.[WONUM],wv.[PlateNumber],wv.[IsAssigned]  ,wv.[CreatedDate] ,wv.[CreatedBy], w.[WODescription],w.[Status_Description] 
            FROM [ODW].[AVLS].[vwWorkOrderVehicleAssignment] wv 
            left join [ODW].[GIS].[vwWorkOrderBase] w
            on wv.[WONUM]=w.[WONUM] where wv.[PlateNumber] = @Title">
            <SelectParameters>
                <asp:QueryStringParameter DbType="String" DefaultValue="8737054" Name="Title" QueryStringField="Title" />
            </SelectParameters>
        </asp:SqlDataSource>

        <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:ODWConnectionString %>"
            SelectCommand="SELECT Count(*) AS TotalRows FROM [ODW].[AVLS].[vwWorkOrderVehicleAssignment] where ([PlateNumber] = @Title)">
            <SelectParameters>
                <asp:QueryStringParameter DbType="String" DefaultValue="8737054" Name="Title" QueryStringField="Title" />
            </SelectParameters>
        </asp:SqlDataSource>
        <asp:GridView ID="gvCustomers" CssClass="Grid" Font-Size="12px" Font-Names="Segoe UI Light" runat="server" AutoGenerateColumns="false" CellPadding="16" CellSpacing="16" DataSourceID="SqlDataSource1" OnRowDeleting="OnRowDeleting" OnRowDataBound="OnRowDataBound">
            <Columns>
                ...
                <asp:BoundField DataField="CreatedBy" HeaderText="CreatedBy" />
                 <asp:BoundField DataField="WODescription" HeaderText="WorkOrder Description" />
                <asp:BoundField DataField="Status_Description" HeaderText="Status Description" />
                <asp:CommandField ShowDeleteButton="True" HeaderText="SELECT" ButtonType="Link" DeleteText="UnAssign" />
            </Columns>
            ...
            <div class="modal-dialog modal-sm">
                <div style="background-color: #022130; font-family: Segoe UI Light; color: white; width: 300px; margin: 0 auto;" class="modal-content">
                    <div class="modal-header" style="padding: 6px; border-bottom: 1px solid #4c636f;">
                        ...
                        <h4 class="modal-title" style="font-weight: bold; font-family: Segoe UI Light; font-size: 14px;">UnAssign WorkOrder: MessageBox</h4>
                    </div>
