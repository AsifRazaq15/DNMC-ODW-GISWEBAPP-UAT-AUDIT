<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="WODetailsBySiteAsset.aspx.cs" Inherits="WebAppForm.WODetailsBySiteAsset" %>
...
<body style="background-color: #022130; font-family: Segoe UI Light; color: white;">
    ...
    <form id="form1" runat="server" style="text-align: center;">
        ...
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:ODWConnectionString %>"
            SelectCommand="SELECT chmb.Facility_Type,wo.EXT_FRAMEZONE, wo.location,chmb.Hierarchical_SiteName, wo.DESCRIPTION,wo.STATUS,wo.WORKTYPE,wo.ASSETNUM, wo.TARGSTARTDATE,wo.TARGCOMPDATE,wo.REPORTEDBY,wo.REPORTDATE,wo.EXT_ENGINEER,wo.CHANGEDATE  
            FROM [SCADA].[Chambers] chmb  with (NOLOCK)  INNER JOIN [EAMS].[FactWorkOrder] wo WITH (NOLOCK) ON wo.ASSETNUM = chmb.Asset_ID            
            WHERE  wo.WORKTYPE = @WORKTYPE and chmb.Asset_ID IS NOT NULL  AND wo.[Status] NOT IN ('CAN','CLOSE','COMP','HISTEDIT','SRVPARRST','THDPTY','WRKCOMP','QC') 
        ...
        <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:ODWConnectionString %>"
            SelectCommand="SELECT COUNT(*) AS TotalRows FROM [SCADA].[Chambers] chmb  with (NOLOCK)  INNER JOIN [EAMS].[FactWorkOrder] wo WITH (NOLOCK) ON wo.ASSETNUM = chmb.Asset_ID
                        WHERE  wo.WORKTYPE = @WORKTYPE and chmb.Asset_ID IS NOT NULL AND wo.[Status] NOT IN ('CAN','CLOSE','COMP','HISTEDIT','SRVPARRST','THDPTY','WRKCOMP','QC')">
            <SelectParameters>
                <asp:ControlParameter ControlID="ddlWorkType" DbType="String" Name="WORKTYPE" PropertyName="SelectedValue" />
            </SelectParameters>
